...
TrackingStage::TrackingStage(TrackingConfig cfg, std::shared_ptr<BoundedQueue<Frame>> in, std::shared_ptr<LatestStore<Detections>> detections_latest_store, std::shared_ptr<BoundedQueue<RenderFrame>> out)
    : Stage("tracking_stage"), cfg_(std::move(cfg)), in_(std::move(in)), detections_latest_store_(std::move(detections_latest_store)), out_(std::move(out)) {}
